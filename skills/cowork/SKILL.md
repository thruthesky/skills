...
> **① 6 AI 병렬 분석**(claude·codex·grok·kimi·agy + OpenCode GLM-5.3, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
1. **여섯 AI 는 읽기 전용으로 물리 차단된다**(claude=`sandbox-exec`+도구 화이트리스트,
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  ...
  · OpenCode=`opencode-go/glm-5.3` **하나만** 명시 고정(2026-08-17 사용자 지시 — 그전의 DeepSeek V4 Pro·MiniMax M3·Qwen 3.6 Plus 세 모델을 GLM-5.3 하나로 교체했다). 별도 `opencode run` 프로세스 하나로 부른다. 🛑 접두어 주의: `opencode-go/`(구독)와 `opencode/`(Zen 종량제)는 다른 경로다 — 틀리면 그 AI 만 "구독에 없다"로 조용히 죽는다. 인증 키는 **① `<작업공간>/.cowork/opencode-go-api.key` → ② `~/.config/cowork/opencode-go-api.key` 순으로 찾아** 먼저 실재하는 쪽을 `OPENCODE_API_KEY` 로 주입한다(①은 분석 대상 폴더 안이라 6 AI 가 읽을 수 있다 — 지원은 하되 실행마다 경고한다. 노출이 곤란하면 ②를 써라).
...
환경변수: `COWORK_TIMEOUT`(AI 당 제한 초, 기본 900) · `COWORK_ONLY`(일부만 재실행, 기본 claude,codex,grok,kimi,glm,agy)
...
· `COWORK_KIMI_MODEL`(kimi-code/k3-256k) · `COWORK_OPENCODE_GLM_MODEL`(opencode-go/glm-5.3; 예전 `COWORK_OPENCODE_MODEL`은 하위 호환 별칭) · `COWORK_OPENCODE_KEYFILE`(미지정 시 `.cowork/opencode-go-api.key` → `~/.config/cowork/opencode-go-api.key` 순 탐색)
...
1. 여섯 파일(`claude-cowork.md`·`codex-cowork.md`·`grok-cowork.md`·`kimi-cowork.md`·`glm-cowork.md`·`agy-cowork.md`)을 모두 읽는다.
...
└── <폴더명>/
    ...
    ├── glm-cowork.md      ← GLM-5.3 분석 (단일 pass · 독립 opencode CLI)
