---
name: cowork
description: 🛑 사용자가 "cowork"·"/cowork"·"코워크"라고 직접 지목했을 때만 사용한다. "분석해줘"·"조사해줘"·"검토해줘"·"원인 파악" 같은 일반 요청으로는 자동 트리거하지 않는다. 하나의 질문을 claude·codex·kimi, GitHub Copilot CLI의 Grok 4.5·MAI-Code-1.1-Flash(별개의 두 프로세스), Antigravity CLI의 Gemini 3.7 Flash High(agy), 총 6 AI에게 동시에 읽기 전용 분석으로 던져 AI별 분석 파일 6개를 만든다. 오케스트레이터가 근거를 직접 교차검증해 `final-report.md`로 종합·보고하며, 요청에 구현·수정이 포함되면 검증된 결론을 즉시 실행한다. 소프트웨어·기획·교재·문제 풀이·레시피·사업 계획 등 모든 분야에서 동작한다. `.cowork/cowork-prompt.md`의 프로젝트 특성·페르소나·규칙을 모든 분석에 주입한다. 사용 시점은 "cowork 해줘"·"/cowork [폴더명] [프롬프트]"·"코워크로 분석해줘", cowork 목록·진행 확인, "/cowork:init"·"cowork 초기화"다. AgentTool·Workflow·deep-research 대신 Bash에서 독립 CLI 프로세스를 실행한다. 키워드 — cowork, 코워크, /cowork, cowork:init.
---

# cowork — 6 AI 교차 분석 → 검증된 결론으로 실행

핵심 흐름 — 이 순서가 스킬의 전부다:

> **① 6 AI 병렬 분석**(claude·codex·copilot-grok·copilot-mai·kimi·agy, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
- 여섯 AI 는 어떤 요청에도 파일을 못 고친다. 고치는 것은 **종합을 마친 오케스트레이터(나) 하나뿐**이다.

## 🛑 절대 규칙

> **🧰 이 스킬은 `AgentTool`(서브에이전트)·Workflow·deep-research 를 쓰지 않는다.** 여섯 AI 는 `Bash` 로
> `scripts/cowork.sh` 를 실행하면 그 스크립트가 각 AI 의 **독립 CLI 프로그램**(`claude -p` ·
> `codex exec` · `copilot -p`×2 · `kimi` · `agy -p`)을 별도 프로세스로 띄우는 것이다(구현: `run_oneshot`·`run_copilot_readonly`·`run_agy`).
...
1. **여섯 AI 는 읽기 전용으로 물리 차단된다**(claude·copilot=`sandbox-exec`+도구 화이트리스트,
   codex=`--sandbox read-only`, kimi·agy=`sandbox-exec`). 방어 수단이 CLI 마다 다른 것은 실측
   결과에 따른 의도다 — kimi·agy 는 CLI 옵션이 무력해 OS 샌드박스로만 막힌다. "통일"하지 말 것.
...
3. **순서 고정: 분석 → 종합 → 실행.** `final-report.md` 없이 무엇도 고치지 말라 — 검증 전 AI 주장은
   환각일 수 있다(예전 grok 2-pass 가 1차 주장을 2차에서 스스로 반증한 실측). 검증 안 된 주장으로 고치면
   없는 문제를 고치거나 멀쩡한 것을 부순다. 분석이 도는 중의 수정도 금지(여섯 AI 가 읽는 자료가
   발밑에서 바뀐다).
...
**이 작업공간이 무엇을 하는 곳인지 여섯 AI 에게 알려주는 파일이다.** 프로젝트의 **특성·기획·계획·
아바타(persona)·디자인·로직·개념·용어·금지사항**을 여기 적어 두면, 이후 **모든 분석의 프롬프트 맨 앞에
반드시 주입**된다. 없으면 여섯 AI 는 목적을 모른 채 일반론으로 분석한다.
...
- 형식 — 네 절이 뼈대이고, 필요하면 절을 더 추가한다(`## 기획`·`## 용어 정의`·`## 설계 규칙` 등):
  ...
  | 절 | 무엇을 적나 |
  |---|---|
  | `## Overview` | 이 프로젝트가 무엇이고 지금 어느 단계인지, 6 AI 에게 무엇을 시키려는지 |
  | `## Persona` | 여섯 AI 가 맡을 전문가 역할(아바타) — 관점 자체가 달라진다 |
...
- **주입 경로**: `cowork.sh` 가 이 파일 전문 + 자동 감지 사실을 합쳐 페르소나의 `{{PROJECT_CONTEXT}}` 에
  치환한다 → 여섯 AI 분석과 리뷰 라운드가 **전부 같은 페르소나 문자열을 쓰므로 자동으로 전파된다.**
...
- **반드시 `run_in_background: true`** — 각 AI 가 수 분씩 걸려 Bash 10분 제한을 넘긴다. 완료 알림이
  오면 3단계로.
- 프롬프트는 **사용자 원문 그대로**(여섯 AI 가 같은 질문을 받아야 대조가 성립). 모호하면 대상 파일·증상
...
- **copilot 은 두뇌 둘을 별개 프로세스로 돌린다**(2026-09-17 사용자 지시 — grok CLI 를 대신한다):
  `copilot-grok`(Grok 4.5)과 `copilot-mai`(MAI-Code-1.1-Flash)가 각자 `copilot -p` 세션을 따로 열어
  **서로의 분석을 보지 못한 채** 분석한다(memory 꺼짐·세션 저장소 도구 차단). 둘 다 단일 pass 다.
  종합 때는 **같은 CLI 에서 나왔다고 한 표로 묶지 말라** — 두 두뇌의 이견이 곧 대조 재료다.
  ⚠️ grok CLI 전용이던 2-pass(자기 비판 지침 `references/grok-deep-protocol.md`)는 grok CLI 와 함께 제거됐다
  (파일은 git 이력에 남아 있다).
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  ...
  기본 low 를 강제 override, **모델 거절 시 `gpt-5.6-terra` 로 1회 fallback**)
  · copilot=`grok-4.5`·`mai-code-1.1-flash` 를 **각각 명시 고정**+`--reasoning-effort high` — 두 모델 모두 `xhigh`·`max` 를 `not supported` 로 거절해 high 가 최고다(2026-09-17 실측, 그 값이 오면 high 로 폴백). `auto` 는 쓰지 않는다(두 두뇌를 따로 쓰는 것이 목적). 실제로 답한 모델이 요청과 다르면(한도 초과 시 auto 전환 등) 그 분석을 **싣지 않고 실패로 기록**한다.
  · **grok CLI 는 2026-09-17 사용자 지시로 제거됐다**(Grok 4.5 두뇌는 copilot 경로로 유지). 되살릴 때의 함정은 [references/readonly-enforcement.md](references/readonly-enforcement.md) §grok CLI(제거됨)에 남겨 두었다.
  · **OpenCode 는 2026-08-17 사용자 지시로 통째로 제거됐다**(그전: DeepSeek·MiniMax·Qwen → GLM-5.3 하나). 되살릴 때의 함정과 실측 근거는 [references/readonly-enforcement.md](references/readonly-enforcement.md) §OpenCode(제거됨)에 남겨 두었다.
...
· `COWORK_COPILOT_GROK_MODEL`(grok-4.5) · `COWORK_COPILOT_MAI_MODEL`(mai-code-1.1-flash) · `COWORK_COPILOT_EFFORT`(high; 두 프로세스 공용, xhigh·max → high 폴백)
...
· `COWORK_AGY_MODEL`(gemini-3.7-flash-high) · `COWORK_AGY_EFFORT`(high; low\|medium\|high 만 지원 → 그 외는 high 폴백)
  — copilot 은 `-p` 에 `--allow-all-tools` 가 필수인데, 그것만 주면 **파일이 그대로 생성된다**(공격 실측).
  그래서 `--available-tools=view,grep,glob` 로 읽기 도구만 보이게 하고 `sandbox-exec` 를 덧씌운다.
  출력은 `--output-format json` 에서 최종 답변만 뽑는다 — text 모드는 도구 호출 사이의 진행 문구가
  섞인다(2026-09-17 실측, `references/readonly-enforcement.md` §copilot).
...
6 AI 분석은 재료일 뿐, 사람이 원하는 산출물은 `final-report.md` 하나다. **완료 알림이 오면 하던 일을
멈추고 종합부터 끝낸다** — 알림은 한 번뿐이고, cowork 두 건을 겹쳐 돌리다 먼저 끝난 쪽의 종합이 통째로
누락된 실제 사고가 있다(2026-07-16). 새 cowork 를 시작하기 전에 `--list` 로 `⏳ 종합대기` 가 없는지
확인한다.

1. 여섯 파일(`claude-cowork.md`·`codex-cowork.md`·`copilot-grok-cowork.md`·`copilot-mai-cowork.md`·`kimi-cowork.md`·`agy-cowork.md`)을 모두 읽는다.
...
- 질문에 대한 최종 답 1~3줄
- 여섯 AI 가 갈렸던 지점과 자료로 내린 판정 · 반증된 주장(있으면)
...
1. 5단계 보고 직후, 곧바로 `bash .claude/skills/cowork/scripts/cowork.sh --review <폴더명>` 을
   `run_in_background: true` 로 실행한다. 리뷰가 6 AI 재검토 → claude 종합으로 `final-report.md` 를
   갱신하고 마커를 지운다(~15분).
...
`final-report.md` 를 **6 AI 에게 한 번 더 던져** 종합본 자체를 비판하게 하고, 그 지적을 반영해
...
- **동작**: cowork 분석이 시작되면 작업폴더에 `.review-final-report` 마커가 생긴다. 메인 Claude 가
  ...
  6 AI 가 `final-report.md` + 6개 원본 분석을 재검토(1-pass) → claude 가 지적을 반영해
...
└── <폴더명>/
    ...
    ├── copilot-grok-cowork.md ← Grok 4.5 분석 (단일 pass · Copilot CLI 프로세스 ①)
    ├── copilot-mai-cowork.md  ← MAI-Code-1.1-Flash 분석 (단일 pass · Copilot CLI 프로세스 ②)
    ...
    ├── .review/           ← 리뷰 라운드 산출물 (6개 리뷰·종합 원출력·원본 백업)
    └── .logs/*.log        ← 각 CLI 원시 로그 (실패 진단용 · copilot 은 이벤트 원본 *.jsonl 도 함께)
```

## 자주 틀리는 지점

- **`.cowork/cowork-prompt.md` 를 안 만들고 쓰는 것** — 여섯 AI 가 프로젝트 목적을 모른 채 일반론을 쓴다.
...
- **종합을 요약으로 대체하는 것** — 근거 검증 없는 종합은 여섯 배로 그럴듯한 환각이다.
