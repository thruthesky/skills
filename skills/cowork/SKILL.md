...
> **① 8 AI 병렬 분석**(claude·codex·grok·kimi + OpenCode 4모델, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
1. **여덟 AI 는 읽기 전용으로 물리 차단된다**(claude=도구 화이트리스트, codex=`--sandbox read-only`,
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  ...
  · OpenCode 네 모델=`opencode-go/deepseek-v4-pro`·`opencode-go/glm-5.2`·`opencode-go/minimax-m3`·`opencode-go/qwen3.6-plus` 명시 고정 — **각각 별도 `opencode run` 프로세스 네 개로 동시에** 부른다. 🛑 접두어 주의: `opencode-go/`(구독)와 `opencode/`(Zen 종량제)는 다른 경로다. 인증은 기본 `~/.config/cowork/opencode-go-api.key` → `OPENCODE_API_KEY` 주입(작업공간 밖 보관).
...
환경변수: `COWORK_TIMEOUT`(AI 당 제한 초, 기본 900) · `COWORK_ONLY`(일부만 재실행, 기본 claude,codex,grok,kimi,deepseek,glm,minimax,qwen)
...
· `COWORK_KIMI_MODEL`(kimi-code/k3-256k) · `COWORK_OPENCODE_DEEPSEEK_MODEL`(opencode-go/deepseek-v4-pro; 예전 `COWORK_OPENCODE_MODEL`은 하위 호환 별칭) · `COWORK_OPENCODE_GLM_MODEL`(opencode-go/glm-5.2) · `COWORK_OPENCODE_MINIMAX_MODEL`(opencode-go/minimax-m3) · `COWORK_OPENCODE_QWEN_MODEL`(opencode-go/qwen3.6-plus) · `COWORK_OPENCODE_KEYFILE`(기본 ~/.config/cowork/opencode-go-api.key)
...
1. 여덟 파일(`claude-cowork.md`·`codex-cowork.md`·`grok-cowork.md`·`kimi-cowork.md`·`deepseek-cowork.md`·`glm-cowork.md`·`minimax-cowork.md`·`qwen-cowork.md`)을 모두 읽는다.
...
└── <폴더명>/
    ...
    ├── glm-cowork.md      ← GLM 5.2 분석 (단일 pass · 독립 opencode CLI)
