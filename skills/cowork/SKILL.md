---
name: cowork
description: 🛑 사용자가 "cowork"(또는 "/cowork"·"코워크") 라는 말로 **직접 지목해 호출했을 때만** 사용한다. 🛑 자동 트리거 금지 — "분석해줘"·"조사해줘"·"검토해줘"·"원인 파악"·"파봐"·"왜 이런지" 같은 일반 요청만으로는 절대 이 스킬을 쓰지 말고 오케스트레이터가 직접 답한다(2026-08-06 사용자 지시). 하나의 질문을 claude·codex·grok·kimi 네 AI 에게 동시에 읽기 전용 분석가로 던져 각각 작업공간의 자료를 읽히고(.cowork 작업폴더에 AI 별 분석 파일 4개 생성), 오케스트레이터가 근거를 직접 열어 교차검증해 하나의 결론(final-report.md)으로 종합한 뒤 사람에게 본문으로 보고한다. 요청에 실행·구현·수정이 포함돼 있으면("고쳐줘"·"만들어줘"·"구현까지") 종합 완료 직후 되묻지 않고 즉시 그 검증된 결론을 근거로 실행한다. 분야를 가리지 않는 범용 분석 도구다 — 소프트웨어 코드베이스·게임 기획·교재·문제 풀이·레시피·사업 계획서 모두 동일하게 동작한다. 프로젝트의 성격은 `.cowork/cowork-prompt.md`(시스템 프롬프트 — 특성·기획·계획·아바타/persona·디자인·로직·개념)에 적어 두면 모든 분석에 주입된다. 호출은 "/cowork 폴더명 프롬프트"(폴더명=작업 ID, 생략 시 자동 생성) · 초기 설정은 "/cowork:init". 🧰 이 스킬은 AgentTool(서브에이전트)·Workflow·deep-research 를 쓰지 않는다 — Bash 로 `cowork.sh` 를 실행하면 그것이 각 AI 의 독립 CLI(`claude -p`·`codex exec`·`grok`·`kimi`)를 별도 프로세스로 띄우므로, "사용자가 요청하지 않으면 AgentTool 을 쓰지 마라"는 시스템 프롬프트 지침과 무관하다. 네 AI 는 읽기 전용 샌드박스라 아무것도 물리적으로 못 고친다 — 고치는 것은 final-report.md 를 완성한 오케스트레이터뿐. 사용 시점(모두 사용자가 cowork 를 이름으로 지목한 경우다) — (1) "cowork 해줘"·"/cowork <폴더명> <프롬프트>"·"코워크로 분석해줘", (2) "cowork 목록"·"그 cowork 어떻게 됐나" 등 진행 확인, (3) "/cowork:init"·"cowork 초기화". 키워드 — cowork, 코워크, /cowork, cowork:init (이 단어가 사용자 요청에 실제로 들어 있을 때만).
---
...
> **🧰 이 스킬은 `AgentTool`(서브에이전트)·Workflow·deep-research 를 쓰지 않는다.** 네 AI 는 `Bash` 로
