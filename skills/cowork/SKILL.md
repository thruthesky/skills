---
name: cowork
description: 🛑 사용자가 "cowork"(또는 "/cowork"·"코워크") 라는 말로 **직접 지목해 호출했을 때만** 사용한다. 🛑 자동 트리거 금지 — "분석해줘"·"조사해줘"·"검토해줘"·"원인 파악"·"파봐"·"왜 이런지" 같은 일반 요청만으로는 절대 이 스킬을 쓰지 말고 오케스트레이터가 직접 답한다(2026-08-06 사용자 지시). 하나의 질문을 claude·codex·grok·kimi·deepseek 다섯 AI 에게 동시에 읽기 전용 분석가로 던져 각각 작업공간의 자료를 읽히고(.cowork 작업폴더에 AI 별 분석 파일 5개 생성), 오케스트레이터가 근거를 직접 열어 교차검증해 하나의 결론(final-report.md)으로 종합한 뒤 사람에게 본문으로 보고한다. 요청에 실행·구현·수정이 포함돼 있으면("고쳐줘"·"만들어줘"·"구현까지") 종합 완료 직후 되묻지 않고 즉시 그 검증된 결론을 근거로 실행한다. 분야를 가리지 않는 범용 분석 도구다 — 소프트웨어 코드베이스·게임 기획·교재·문제 풀이·레시피·사업 계획서 모두 동일하게 동작한다. 프로젝트의 성격은 `.cowork/cowork-prompt.md`(시스템 프롬프트 — 특성·기획·계획·아바타/persona·디자인·로직·개념)에 적어 두면 모든 분석에 주입된다. 호출은 "/cowork 폴더명 프롬프트"(폴더명=작업 ID, 생략 시 자동 생성) · 초기 설정은 "/cowork:init". 🧰 이 스킬은 AgentTool(서브에이전트)·Workflow·deep-research 를 쓰지 않는다 — Bash 로 `cowork.sh` 를 실행하면 그것이 각 AI 의 독립 CLI(`claude -p`·`codex exec`·`grok`·`kimi`·`opencode run`)를 별도 프로세스로 띄우므로, "사용자가 요청하지 않으면 AgentTool 을 쓰지 마라"는 시스템 프롬프트 지침과 무관하다. 다섯 AI 는 읽기 전용 샌드박스라 아무것도 물리적으로 못 고친다 — 고치는 것은 final-report.md 를 완성한 오케스트레이터뿐. 사용 시점(모두 사용자가 cowork 를 이름으로 지목한 경우다) — (1) "cowork 해줘"·"/cowork <폴더명> <프롬프트>"·"코워크로 분석해줘", (2) "cowork 목록"·"그 cowork 어떻게 됐나" 등 진행 확인, (3) "/cowork:init"·"cowork 초기화". 키워드 — cowork, 코워크, /cowork, cowork:init (이 단어가 사용자 요청에 실제로 들어 있을 때만).
---

# cowork — 5 AI 교차 분석 → 검증된 결론으로 실행

핵심 흐름 — 이 순서가 스킬의 전부다:

> **① 5 AI 병렬 분석**(claude·codex·grok·kimi·deepseek, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
- 다섯 AI 는 어떤 요청에도 파일을 못 고친다. 고치는 것은 **종합을 마친 오케스트레이터(나) 하나뿐**이다.

## 🛑 절대 규칙

> **🧰 이 스킬은 `AgentTool`(서브에이전트)·Workflow·deep-research 를 쓰지 않는다.** 다섯 AI 는 `Bash` 로
> `scripts/cowork.sh` 를 실행하면 그 스크립트가 각 AI 의 **독립 CLI 프로그램**(`claude -p` ·
> `codex exec` · `grok` · `kimi`)을 별도 프로세스로 띄우는 것이다(구현: `run_oneshot`·`run_grok_headless`).
...
1. **다섯 AI 는 읽기 전용으로 물리 차단된다**(claude=도구 화이트리스트, codex=`--sandbox read-only`,
...
3. **순서 고정: 분석 → 종합 → 실행.** `final-report.md` 없이 무엇도 고치지 말라 — 검증 전 AI 주장은
   ...
   없는 문제를 고치거나 멀쩡한 것을 부순다. 분석이 도는 중의 수정도 금지(다섯 AI 가 읽는 자료가
   발밑에서 바뀐다).
...
**이 작업공간이 무엇을 하는 곳인지 다섯 AI 에게 알려주는 파일이다.** 프로젝트의 **특성·기획·계획·
아바타(persona)·디자인·로직·개념·용어·금지사항**을 여기 적어 두면, 이후 **모든 분석의 프롬프트 맨 앞에
반드시 주입**된다. 없으면 다섯 AI 는 목적을 모른 채 일반론으로 분석한다.
...
- 형식 — 네 절이 뼈대이고, 필요하면 절을 더 추가한다(`## 기획`·`## 용어 정의`·`## 설계 규칙` 등):
  ...
  | 절 | 무엇을 적나 |
  |---|---|
  | `## Overview` | 이 프로젝트가 무엇이고 지금 어느 단계인지, 5 AI 에게 무엇을 시키려는지 |
  | `## Persona` | 다섯 AI 가 맡을 전문가 역할(아바타) — 관점 자체가 달라진다 |
...
- **반드시 `run_in_background: true`** — 각 AI 가 수 분씩 걸려 Bash 10분 제한을 넘긴다. 완료 알림이
  오면 3단계로.
- 프롬프트는 **사용자 원문 그대로**(다섯 AI 가 같은 질문을 받아야 대조가 성립). 모호하면 대상 파일·증상
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  ...
  · deepseek=`opencode-go/deepseek-v4-flash` 명시 고정 — OpenCode Go 구독(정액)을 `opencode run` 으로 부른다. 🛑 접두어 주의: `opencode-go/`(구독)와 `opencode/`(Zen 종량제, 단가 약 4배)는 다른 경로다. 인증은 `.cowork/opencode-go-api.key` → `OPENCODE_API_KEY` 주입(**.gitignore 필수**).
...
환경변수: `COWORK_TIMEOUT`(AI 당 제한 초, 기본 900) · `COWORK_ONLY`(일부만 재실행, 기본 claude,codex,grok,kimi,deepseek)
...
· `COWORK_KIMI_MODEL`(kimi-code/k3-256k) · `COWORK_OPENCODE_MODEL`(opencode-go/deepseek-v4-flash) · `COWORK_OPENCODE_KEYFILE`(.cowork/opencode-go-api.key)
...
5 AI 분석은 재료일 뿐, 사람이 원하는 산출물은 `final-report.md` 하나다. **완료 알림이 오면 하던 일을
멈추고 종합부터 끝낸다** — 알림은 한 번뿐이고, cowork 두 건을 겹쳐 돌리다 먼저 끝난 쪽의 종합이 통째로
누락된 실제 사고가 있다(2026-07-16). 새 cowork 를 시작하기 전에 `--list` 로 `⏳ 종합대기` 가 없는지
확인한다.

1. 다섯 파일(`claude-cowork.md`·`codex-cowork.md`·`grok-cowork.md`·`kimi-cowork.md`·`deepseek-cowork.md`)을 모두 읽는다.
...
- 질문에 대한 최종 답 1~3줄
- 다섯 AI 가 갈렸던 지점과 자료로 내린 판정 · 반증된 주장(있으면)
...
1. 5단계 보고 직후, 곧바로 `bash .claude/skills/cowork/scripts/cowork.sh --review <폴더명>` 을
   `run_in_background: true` 로 실행한다. 리뷰가 5 AI 재검토 → claude 종합으로 `final-report.md` 를
   갱신하고 마커를 지운다(~15분).
...
`final-report.md` 를 **5 AI 에게 한 번 더 던져** 종합본 자체를 비판하게 하고, 그 지적을 반영해
...
- **동작**: cowork 분석이 시작되면 작업폴더에 `.review-final-report` 마커가 생긴다. 메인 Claude 가
  ...
  5 AI 가 `final-report.md` + 5개 원본 분석을 재검토(1-pass) → claude 가 지적을 반영해
...
└── <폴더명>/
    ...
    ├── deepseek-cowork.md ← deepseek 분석 (단일 pass, DeepSeek V4 Flash · opencode CLI 경유)
    ...
    ├── .review/           ← 리뷰 라운드 산출물 (5개 리뷰·종합 원출력·원본 백업)
    └── .logs/*.log        ← 각 CLI 원시 로그 (실패 진단용)
```

## 자주 틀리는 지점

- **`.cowork/cowork-prompt.md` 를 안 만들고 쓰는 것** — 다섯 AI 가 프로젝트 목적을 모른 채 일반론을 쓴다.
