---
name: cowork
description: 🛑 사용자가 "cowork"·"/cowork"·"코워크"라고 직접 지목했을 때만 사용한다. "분석해줘"·"조사해줘"·"검토해줘"·"원인 파악" 같은 일반 요청으로는 자동 트리거하지 않는다. 하나의 질문을 claude·codex·grok·kimi와 OpenCode의 DeepSeek V4 Pro·MiniMax M3·Qwen 3.6 Plus, 총 7 AI에게 동시에 읽기 전용 분석으로 던진다. OpenCode 3모델은 각각 별도 `opencode run` 프로세스로 병렬 실행하고, AI별 분석 파일 8개를 만든다. 오케스트레이터가 근거를 직접 교차검증해 `final-report.md`로 종합·보고하며, 요청에 구현·수정이 포함되면 검증된 결론을 즉시 실행한다. 소프트웨어·기획·교재·문제 풀이·레시피·사업 계획 등 모든 분야에서 동작한다. `.cowork/cowork-prompt.md`의 프로젝트 특성·페르소나·규칙을 모든 분석에 주입한다. 사용 시점은 "cowork 해줘"·"/cowork [폴더명] [프롬프트]"·"코워크로 분석해줘", cowork 목록·진행 확인, "/cowork:init"·"cowork 초기화"다. AgentTool·Workflow·deep-research 대신 Bash에서 독립 CLI 프로세스를 실행한다. 키워드 — cowork, 코워크, /cowork, cowork:init.
---

# cowork — 7 AI 교차 분석 → 검증된 결론으로 실행

핵심 흐름 — 이 순서가 스킬의 전부다:

> **① 7 AI 병렬 분석**(claude·codex·grok·kimi + OpenCode 3모델, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
- 일곱 AI 는 어떤 요청에도 파일을 못 고친다. 고치는 것은 **종합을 마친 오케스트레이터(나) 하나뿐**이다.

## 🛑 절대 규칙

> **🧰 이 스킬은 `AgentTool`(서브에이전트)·Workflow·deep-research 를 쓰지 않는다.** 일곱 AI 는 `Bash` 로
> `scripts/cowork.sh` 를 실행하면 그 스크립트가 각 AI 의 **독립 CLI 프로그램**(`claude -p` ·
> `codex exec` · `grok` · `kimi` · `opencode run`×4)을 별도 프로세스로 띄우는 것이다(구현: `run_oneshot`·`run_grok_headless`·`run_opencode_analysis`).
...
1. **일곱 AI 는 읽기 전용으로 물리 차단된다**(claude=도구 화이트리스트, codex=`--sandbox read-only`,
   grok·kimi·OpenCode=`sandbox-exec`). 방어 수단이 CLI 마다 다른 것은 실측 결과에 따른 의도다 — grok·kimi·OpenCode 는
...
3. **순서 고정: 분석 → 종합 → 실행.** `final-report.md` 없이 무엇도 고치지 말라 — 검증 전 AI 주장은
   ...
   없는 문제를 고치거나 멀쩡한 것을 부순다. 분석이 도는 중의 수정도 금지(일곱 AI 가 읽는 자료가
   발밑에서 바뀐다).
...
**이 작업공간이 무엇을 하는 곳인지 일곱 AI 에게 알려주는 파일이다.** 프로젝트의 **특성·기획·계획·
아바타(persona)·디자인·로직·개념·용어·금지사항**을 여기 적어 두면, 이후 **모든 분석의 프롬프트 맨 앞에
반드시 주입**된다. 없으면 일곱 AI 는 목적을 모른 채 일반론으로 분석한다.
...
- 형식 — 네 절이 뼈대이고, 필요하면 절을 더 추가한다(`## 기획`·`## 용어 정의`·`## 설계 규칙` 등):
  ...
  | 절 | 무엇을 적나 |
  |---|---|
  | `## Overview` | 이 프로젝트가 무엇이고 지금 어느 단계인지, 7 AI 에게 무엇을 시키려는지 |
  | `## Persona` | 일곱 AI 가 맡을 전문가 역할(아바타) — 관점 자체가 달라진다 |
...
- **반드시 `run_in_background: true`** — 각 AI 가 수 분씩 걸려 Bash 10분 제한을 넘긴다. 완료 알림이
  오면 3단계로.
- 프롬프트는 **사용자 원문 그대로**(일곱 AI 가 같은 질문을 받아야 대조가 성립). 모호하면 대상 파일·증상
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  ...
  · OpenCode 세 모델=`opencode-go/deepseek-v4-pro`·`opencode-go/minimax-m3`·`opencode-go/qwen3.6-plus` 명시 고정 — **각각 별도 `opencode run` 프로세스 세 개로 동시에** 부른다. 🛑 접두어 주의: `opencode-go/`(구독)와 `opencode/`(Zen 종량제)는 다른 경로다. 인증은 기본 `~/.config/cowork/opencode-go-api.key` → `OPENCODE_API_KEY` 주입(작업공간 밖 보관).
...
환경변수: `COWORK_TIMEOUT`(AI 당 제한 초, 기본 900) · `COWORK_ONLY`(일부만 재실행, 기본 claude,codex,grok,kimi,deepseek,minimax,qwen)
...
· `COWORK_KIMI_MODEL`(kimi-code/k3-256k) · `COWORK_OPENCODE_DEEPSEEK_MODEL`(opencode-go/deepseek-v4-pro; 예전 `COWORK_OPENCODE_MODEL`은 하위 호환 별칭) · `COWORK_OPENCODE_MINIMAX_MODEL`(opencode-go/minimax-m3) · `COWORK_OPENCODE_QWEN_MODEL`(opencode-go/qwen3.6-plus) · `COWORK_OPENCODE_KEYFILE`(기본 ~/.config/cowork/opencode-go-api.key)
...
7 AI 분석은 재료일 뿐, 사람이 원하는 산출물은 `final-report.md` 하나다. **완료 알림이 오면 하던 일을
멈추고 종합부터 끝낸다** — 알림은 한 번뿐이고, cowork 두 건을 겹쳐 돌리다 먼저 끝난 쪽의 종합이 통째로
누락된 실제 사고가 있다(2026-07-16). 새 cowork 를 시작하기 전에 `--list` 로 `⏳ 종합대기` 가 없는지
확인한다.

1. 일곱 파일(`claude-cowork.md`·`codex-cowork.md`·`grok-cowork.md`·`kimi-cowork.md`·`deepseek-cowork.md`·`minimax-cowork.md`·`qwen-cowork.md`)을 모두 읽는다.
...
- 질문에 대한 최종 답 1~3줄
- 일곱 AI 가 갈렸던 지점과 자료로 내린 판정 · 반증된 주장(있으면)
...
1. 5단계 보고 직후, 곧바로 `bash .claude/skills/cowork/scripts/cowork.sh --review <폴더명>` 을
   `run_in_background: true` 로 실행한다. 리뷰가 7 AI 재검토 → claude 종합으로 `final-report.md` 를
   갱신하고 마커를 지운다(~15분).
...
`final-report.md` 를 **7 AI 에게 한 번 더 던져** 종합본 자체를 비판하게 하고, 그 지적을 반영해
...
- **동작**: cowork 분석이 시작되면 작업폴더에 `.review-final-report` 마커가 생긴다. 메인 Claude 가
  ...
  7 AI 가 `final-report.md` + 7개 원본 분석을 재검토(1-pass) → claude 가 지적을 반영해
...
- **`.cowork/cowork-prompt.md` 를 안 만들고 쓰는 것** — 일곱 AI 가 프로젝트 목적을 모른 채 일반론을 쓴다.
