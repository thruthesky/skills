---
name: cowork
description: 사람이 분석·조사·원인 파악을 요청하면 혼자 답하지 말고 이 스킬을 쓴다. 하나의 질문을 claude·codex·grok·kimi 네 AI 에게 동시에 읽기 전용 분석가로 던져 각각 코드를 읽히고(.cowork 작업폴더에 AI 별 분석 파일 4개 생성), 오케스트레이터가 근거를 코드로 교차검증해 하나의 결론(final-report.md)으로 종합한 뒤 사람에게 본문으로 보고한다. 사용자 요청에 구현·수정이 포함돼 있으면("고쳐줘"·"구현까지") 종합 완료 직후 되묻지 않고 즉시 그 검증된 결론을 근거로 구현한다. 프로젝트 종류 무관(스택·지침 문서를 실행 시점 자동 감지·주입). 호출은 "/cowork 폴더명 프롬프트"(폴더명=작업 ID, 생략 시 자동 생성). 🛑 네 AI 는 읽기 전용 샌드박스라 코드를 물리적으로 못 고친다 — 고치는 것은 final-report.md 를 완성한 오케스트레이터뿐. 다음 경우 사용 — (1) "cowork 해줘", "/cowork", "파봐", (2) "왜 이런지 분석"·"원인 파악"·"조사해줘"·"어떻게 동작하나"·"이 설계 맞나"·"영향 범위" 등 답이 코드 안에 있는 질문 전부, (3) 원인 불명 버그·렉·회귀의 다각도 분석, (4) 설계 판단의 교차 검증, (5) 큰 변경 전 영향·리스크 조사, (6) "cowork 해서 고쳐줘"처럼 분석 후 구현까지, (7) "cowork 목록"·"그 분석 어떻게 됐나" 등 진행 확인. 키워드 — cowork, 코워크, 분석해줘, 원인 분석, 조사해줘, 파봐, 딥다이브, 심층 분석, 4 AI 분석, 3 AI 분석, 교차 검증, 크로스 체크, claude codex grok kimi, 분석만, 분석하고 고쳐줘, final-report, cowork 목록, 작업 ID.
---

# cowork — 4 AI 교차 분석 → 검증된 결론으로 구현

핵심 흐름 — 이 순서가 스킬의 전부다:

> **① 4 AI 병렬 분석**(claude·codex·grok·kimi, 읽기 전용) → **② 코드로 교차검증해 `final-report.md` 종합**
> → **③ 사람에게 본문 보고** → **④ 요청에 구현이 포함돼 있으면 즉시 구현** (되묻지 않는다)

- 폴더명 = 작업 ID. 산출물은 `.cowork/<폴더명>/` 에 쌓인다(§1단계).
- 프로젝트를 가리지 않는다 — 저장소 이름·스택·지침 문서는 `cowork.sh` 가 실행 시점에 감지해 주입한다.
  특정 프로젝트를 페르소나에 하드코딩하지 말라(다른 저장소에서 없는 파일을 찾아 헤맨 실측 사고).
- 네 AI 는 어떤 요청에도 코드를 못 고친다. 고치는 것은 **종합을 마친 오케스트레이터(나) 하나뿐**이다.
...
3. **순서 고정: 분석 → 종합 → 구현.** `final-report.md` 없이 코드를 고치지 말라 — 검증 전 AI 주장은
   환각일 수 있다(grok 이 1차 주장을 2차에서 스스로 반증한 실측). 검증 안 된 주장으로 고치면
   없는 문제를 고치거나 멀쩡한 코드를 부순다. 분석이 도는 중의 수정도 금지(네 AI 가 읽는 코드가
   발밑에서 바뀐다).
4. **종합은 검증이지 요약이 아니다.** 근거 `파일:줄` 을 직접 열어 판정하고, 다수결로 판정하지 않는다
   (2:1 이어도 코드가 소수를 지지하면 소수가 맞다).
...
| 입력 | 폴더명 | 프롬프트 |
|---|---|---|
| `/cowork mob-crowd-fps 몹이 몰리면 FPS 가 떨어진다` | `mob-crowd-fps` | 몹이 몰리면 FPS 가 떨어진다 |
| `/cowork 몹이 몰리면 FPS 가 떨어진다` | *(자동 생성)* | 전체 |
| `/cowork autopilot 이 왜 멈추나` | *(자동 생성)* | 전체 — `autopilot` 을 빼면 문장이 깨진다. 폴더명이 아니라 주어다 |

- **없으면 자동 생성**: 주제를 영문 kebab-case 2~4단어로(예: `party-invite-fail`). 자동 생성했으면
...
- **모델은 항상 최신/최고 등급으로 고정된다**(2026-07-21 사용자 지시 — 낮추지 말 것):
  claude=`claude-opus-4-8`(Opus 4.8 고정)+`--effort xhigh` · codex=config 최신 모델+`model_reasoning_effort=xhigh`(config
  ...
  claude 는 별칭 `opus`(=최신 opus 추종)가 아니라 전체 ID 로 4.8 을 못 박는다(2026-07-22 사용자 지시).
...
· `COWORK_CLAUDE_MODEL`/`COWORK_CLAUDE_EFFORT`(claude-opus-4-8/xhigh) · `COWORK_CODEX_MODEL`/`COWORK_CODEX_EFFORT`(config 상속/xhigh)
...
4. `.cowork/<폴더명>/final-report.md` 를
...
- 질문에 대한 최종 답 1~3줄
- 네 AI 가 갈렸던 지점과 코드로 내린 판정 · 반증된 주장(있으면)
- 권고 1~3순위와 각각의 범위·리스크
- 파일 위치 `.cowork/<폴더명>/final-report.md` (자동 생성 폴더명이면 그 사실도)

**요청에 구현이 포함돼 있었으면 여기서 멈추지 말고 곧바로 6단계로 간다** — "진행할까요?" 라고 묻지
않는다. 분석만 요청받았으면 "수정은 하지 않았다"를 남기고 종료한다.

## 6단계 — 리뷰 후 즉시 구현 〔요청에 구현이 포함된 경우〕

| 사용자 요청 | 어디까지 |
|---|---|
| "고쳐줘"·"수정까지"·"구현해줘"·"분석하고 적용해" | 6단계 — 보고 직후 **즉시 구현** |
| "분석해줘"·"왜 이런지 파봐"·"고치지 말고" | 5단계에서 종료 |
| 애매하면 | 5단계에서 종료 — 사용자가 지시하면 그때 구현 |

#### 🛑 구현 전에 최종 리뷰 라운드를 *먼저* 돌린다 (리뷰 후 구현)

구현은 **리뷰 라운드로 갱신된 최종 `final-report.md`** 를 근거로 해야 한다. 그런데 Stop hook 의 리뷰는
turn 종료 *후* 백그라운드로 도므로, 그냥 두면 구현이 리뷰보다 먼저 일어난다. 그래서 구현 요청일 때는
...
1. 5단계 보고 직후, 곧바로 `bash .claude/skills/cowork/scripts/cowork.sh --review <폴더명>` 을
   ...
   갱신하고 마커를 지운다(~15분).
2. 완료 알림을 받으면 **갱신된** `final-report.md` 로 6단계 구현을 진행한다(리뷰 전 버전이 아니라 최종본).
...
4. **예외**: 사용자가 "리뷰 없이 고쳐"·"바로 고쳐" 라고 하거나, 그 프로젝트에 리뷰 hook 이 설치되지
   않았고 사용자가 리뷰를 원치 않으면, 이 단계를 건너뛰고 현재 `final-report.md` 로 구현한다.

구현 규칙:

1. **(리뷰로 갱신된) `final-report.md` §7 최종 권고가 작업 지시서다** — 우선순위대로 적용한다.
2. **§6 반증된 주장 기반 수정 금지. 권고에 없는 임의 수정 금지**(필요해지면 보고에 왜인지 밝힌다).
3. **그 프로젝트의 차단지점·검증 규칙을 따른다** — 무엇이 차단지점(운영 배포·destructive git 등)이고
   어떻게 검증(실행·스샷·테스트)하는지는 저장소 지침 문서(`CLAUDE.md` 등)가 정한다. 검증 없이 "완료"
   금지.
4. 검증 통과 후 **커밋**(메시지에 cowork 폴더명을 남겨 근거를 추적 가능하게).
5. `final-report.md` 끝에 **`## 9. 적용 결과` 를 덧붙인다**(§1~8 은 수정 전 사실의 기록이므로 보존)
   — 적용/보류 내역, 커밋 해시, 검증 결과.

보고: 무엇을 고쳤나(파일) · 어떻게 검증했나 · 커밋 해시 · 보류한 권고와 이유 · 서버 배포/클라 재빌드
필요 여부.
...
- **설치**(1회): `bash .claude/skills/cowork/scripts/cowork.sh --init` — 이 프로젝트
  `.claude/settings.json` 의 Stop hook 에 `final-report-stop-hook.sh` 를 멱등 등록한다(기존 hook 보존).
  사용자가 "/cowork --init" 또는 "리뷰 hook 설치" 라고 하면 이 명령을 실행한다.
- **동작**: cowork 분석이 시작되면 작업폴더에 `.review-final-report` 마커가 생긴다. 메인 Claude 가
  ...
  단 **구현까지 요청받았으면** hook 의 백그라운드 리뷰를 기다리지 말고, 오케스트레이터가 `--review` 를
  동기로 먼저 돌린 뒤 그 최종본으로 구현한다(§6단계 — "리뷰 후 구현").
...
- **구현 요청인데 보고 후 멈추는 것** — "고쳐줘"를 받았으면 되묻지 말고 즉시 6단계로 간다.
- **순서를 뒤집는 것** — 구현 요청이어도 분석 → 종합 → 구현이다. final-report.md 가 먼저다.
- **종합을 요약으로 대체하는 것** — 근거 검증 없는 종합은 세 배로 그럴듯한 환각이다.
- **다수결 판정** — 코드가 판정한다, 표수가 아니라.
