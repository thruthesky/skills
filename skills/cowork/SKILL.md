---
name: cowork
description: 🛑 사용자가 "cowork"·"/cowork"·"코워크"라고 직접 지목했을 때만 사용한다. "분석해줘"·"조사해줘"·"검토해줘"·"원인 파악" 같은 일반 요청으로는 자동 트리거하지 않는다. 하나의 질문을 claude·codex·grok·kimi와 Antigravity CLI의 Gemini 3.7 Flash High(agy), 총 5 AI에게 동시에 읽기 전용 분석으로 던져 AI별 분석 파일 5개를 만든다. 오케스트레이터가 근거를 직접 교차검증해 `final-report.md`로 종합·보고하며, 요청에 구현·수정이 포함되면 검증된 결론을 즉시 실행한다. 소프트웨어·기획·교재·문제 풀이·레시피·사업 계획 등 모든 분야에서 동작한다. `.cowork/cowork-prompt.md`의 프로젝트 특성·페르소나·규칙을 모든 분석에 주입한다. 사용 시점은 "cowork 해줘"·"/cowork [폴더명] [프롬프트]"·"코워크로 분석해줘", cowork 목록·진행 확인, "/cowork:init"·"cowork 초기화"다. AgentTool·Workflow·deep-research 대신 Bash에서 독립 CLI 프로세스를 실행한다. 키워드 — cowork, 코워크, /cowork, cowork:init.
---
...
> **① 5 AI 병렬 분석**(claude·codex·grok·kimi·agy, 읽기 전용) → **② 근거를 직접 열어 교차검증해 `final-report.md` 종합**
...
> `codex exec` · `grok` · `kimi` · `agy -p`)을 별도 프로세스로 띄우는 것이다(구현: `run_oneshot`·`run_grok_headless`·`run_agy`).
...
1. **다섯 AI 는 읽기 전용으로 물리 차단된다**(claude=`sandbox-exec`+도구 화이트리스트,
   codex=`--sandbox read-only`, grok·kimi·agy=`sandbox-exec`). 방어 수단이 CLI 마다 다른 것은 실측
   결과에 따른 의도다 — grok·kimi·agy 는 CLI 옵션이 무력해 OS 샌드박스로만 막힌다. "통일"하지 말 것.
...
환경변수: `COWORK_TIMEOUT`(AI 당 제한 초, 기본 900) · `COWORK_ONLY`(일부만 재실행, 기본 claude,codex,grok,kimi,agy)
...
· `COWORK_KIMI_MODEL`(kimi-code/k3-256k) · `COWORK_AGY_MODEL`(gemini-3.7-flash-high) · `COWORK_AGY_EFFORT`(high)
...
1. 다섯 파일(`claude-cowork.md`·`codex-cowork.md`·`grok-cowork.md`·`kimi-cowork.md`·`agy-cowork.md`)을 모두 읽는다.
