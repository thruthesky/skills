...
- **목표는 "전체 파일시스템 읽기 전용" 이 아니라 "프로젝트 코드 불변"** 이다. 그래서 repo subpath 만
...
- **종합 단계(오케스트레이터)** 는 `Read`/`Grep`/`Glob` 만 쓰도록 SKILL.md 가 규정한다. 이것은 규칙이지
  샌드박스가 아니므로, cowork 결과를 보고 곧바로 코드를 고치지 않도록 주의한다.
