...
grok·kimi·agy 와 **완전히 같은 프로파일**을 쓴다. 실측에서 claude 는 이 샌드박스 아래서 정상
...
> ⚠️ 이 절은 **OpenCode 제거와 무관하게 계속 유효하다.** `run_timeout()` 은 claude·codex·grok·kimi·agy
...
부수 실측 — 정상 경로에서 이 조합은 온전히 동작한다: 5 AI 통합 검증 실행에서 agy 는 103초에 11.1KB,
