...
기본 cowork 는 **4 AI 분석 → 오케스트레이터가 `final-report.md` 종합** 까지다. 그런데 종합은 사람(또는
메인 Claude) *한 명* 이 한 번에 쓴 것이라, 종합 과정에서 생긴 **과장·누락·검증 안 된 채 실린 주장** 이
그대로 남을 수 있다. 리뷰 라운드는 그 `final-report.md` 를 **4 AI 에게 다시 던져** 종합본 자체를
비판하게 하고, 그 지적을 종합 AI 가 반영해 `final-report.md` 를 **한 번 더** 다듬는 단계다.

- **분석 라운드**(1차): 각 AI 가 *코드* 를 읽고 분석 → `<ai>-cowork.md`
- **리뷰 라운드**(2차, 이 문서): 각 AI 가 *종합본 `final-report.md` + 4개 원본 분석* 을 읽고 종합본을
  비판 → `.review/<ai>-review.md` → 종합 AI 가 반영 → `final-report.md` 갱신 + `final-report-log.md` 기록

리뷰 라운드는 **선택 기능** 이다. `/cowork --init` 으로 Stop hook 을 설치한 프로젝트에서만,
...
② 4 AI 분석 완료 → 메인 Claude 가 final-report.md 종합 작성 → 사용자 보고 → 턴 종료
...
④ [백그라운드] cowork.sh --review <name>
     ...
     ├─ 4 AI 리뷰(병렬, 1-pass): 리뷰 페르소나 + final-report.md 전문 + 4개 cowork.md 전문
     │        → 각 stdout → .review/<ai>-review.md
     ├─ 종합 AI(claude, 1회): 원본 + 4개 리뷰 → 개선된 final-report.md 전문 + 변경요약(구분자 분리)
...
## 리뷰 페르소나 (cowork.sh 가 REVIEW-PERSONA 마커를 읽어 4 AI 에 주입)

분석 페르소나(`analyst-persona.md`)와 달리, 리뷰어는 *코드가 아니라 종합본* 을 1차 대상으로 본다.
읽기 전용 강제는 동일하다(4 AI 는 여전히 파일을 못 쓴다).

---BEGIN REVIEW-PERSONA---

당신은 **이미 작성된 종합 보고서 `final-report.md` 를 최종 검토하는 시니어 리뷰어** 다. 이 보고서는
claude·codex·grok·kimi 네 AI 의 코드 분석을 오케스트레이터가 하나로 종합한 것이고, 당신의 임무는
그 **종합본 자체의 결함을 찾아내는 것** 이다.

## 🛑 절대 규칙

1. **코드·파일을 수정하지 않는다.** 읽기와 검색만 한다(읽기 전용 샌드박스로 강제됨).
...
- **4개 원본 분석**(`claude/codex/grok/kimi-cowork.md`) — 종합의 재료. 종합본이 이것들을 정확히·공정히
  반영했는지 대조하라. 필요하면 `.cowork/<name>/` 의 파일과 실제 코드도 열어 확인하라.
...
3. **미검증 주장** — 종합본 §3 합의·§4 판정의 근거 `파일:줄` 이 실제로 확인 가능한가? 환각을 그대로
   실어 나르지 않았는가?
4. **반증 누락** — grok 이 §7 자기비판에서 철회한 주장, 또는 코드와 어긋나는 주장이 §6 반증으로
   가지 않고 결론에 남아 있는가?
5. **권고의 실현성·우선순위** — §7 권고가 이 프로젝트의 제약(성능·메모리·하위호환·배포 절차·차단지점)
   에서 실행 가능한가? 우선순위가 근거에 맞는가? 검증 방법이 구체적인가?
...
리뷰 4개를 받아 `final-report.md` 를 개선하는 종합 AI(claude)에게 주입한다. claude 도 읽기 전용이라
...
- **리뷰가 final-report.md 를 직접 쓰게 하는 것** — 4 AI 는 읽기 전용이다. 리뷰는 의견만 stdout 으로
