...
**리뷰는 전부 1-pass 다**: 리뷰 입력은 *이미 종합된 결론* 이고 리뷰 자체가 비판 작업이다. copilot 은
분석 때와 똑같이 **두 두뇌(copilot-grok·copilot-mai)를 별개 프로세스로** 띄워 각자 리뷰한다.
...
claude·codex·copilot-grok·copilot-mai·kimi·agy 여섯 AI 의 분석을 오케스트레이터가 하나로 종합한 것이고, 당신의 임무는
...
- **6개 원본 분석**(`claude/codex/copilot-grok/copilot-mai/kimi/agy-cowork.md`) — 종합의 재료. 종합본이 이것들을 정확히·공정히
...
3. **미검증 주장** — 종합본 §3 합의·§4 판정의 근거 `파일:줄` 이 실제로 확인 가능한가? 환각을 그대로
   실어 나르지 않았는가?
4. **반증 누락** — 한 AI 가 근거를 들어 반박한 주장, 또는 실제 자료와 어긋나는 주장이 §6 반증으로
   가지 않고 결론에 남아 있는가?
...
└── .review/
    ...
    ├── copilot-grok-review.md   ← Grok 4.5(Copilot CLI) 리뷰 의견
    ├── copilot-mai-review.md    ← MAI-Code-1.1-Flash(Copilot CLI) 리뷰 의견
...
> 6 AI 리뷰(claude·codex·copilot-grok·copilot-mai·kimi·agy) → claude 종합 → final-report.md 갱신
