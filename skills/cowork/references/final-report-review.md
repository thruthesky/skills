...
기본 cowork 는 **5 AI 분석 → 오케스트레이터가 `final-report.md` 종합** 까지다. 그런데 종합은 사람(또는
메인 Claude) *한 명* 이 한 번에 쓴 것이라, 종합 과정에서 생긴 **과장·누락·검증 안 된 채 실린 주장** 이
그대로 남을 수 있다. 리뷰 라운드는 그 `final-report.md` 를 **5 AI 에게 다시 던져** 종합본 자체를
비판하게 하고, 그 지적을 종합 AI 가 반영해 `final-report.md` 를 **한 번 더** 다듬는 단계다.

- **분석 라운드**(1차): 각 AI 가 *작업공간 자료* 를 읽고 분석 → `<ai>-cowork.md`
- **리뷰 라운드**(2차, 이 문서): 각 AI 가 *종합본 `final-report.md` + 5개 원본 분석* 을 읽고 종합본을
...
② 5 AI 분석 완료 → 메인 Claude 가 final-report.md 종합 작성 → 사용자 보고 → 턴 종료
...
④ [백그라운드] cowork.sh --review <name>
     ...
     ├─ 5 AI 리뷰(병렬, 1-pass): 리뷰 페르소나 + final-report.md 전문 + 5개 cowork.md 전문
     │        → 각 stdout → .review/<ai>-review.md
     ├─ 종합 AI(claude, 1회): 원본 + 5개 리뷰 → 개선된 final-report.md 전문 + 변경요약(구분자 분리)
...
## 리뷰 페르소나 (cowork.sh 가 REVIEW-PERSONA 마커를 읽어 5 AI 에 주입)

분석 페르소나(`analyst-persona.md`)와 달리, 리뷰어는 *작업공간 자료가 아니라 종합본* 을 1차 대상으로
본다. 읽기 전용 강제는 동일하다(5 AI 는 여전히 파일을 못 쓴다).
...
claude·codex·grok·kimi·deepseek 다섯 AI 의 분석을 오케스트레이터가 하나로 종합한 것이고, 당신의 임무는
...
- **5개 원본 분석**(`claude/codex/grok/kimi/deepseek-cowork.md`) — 종합의 재료. 종합본이 이것들을 정확히·공정히
...
리뷰 5개를 받아 `final-report.md` 를 개선하는 종합 AI(claude)에게 주입한다. claude 도 읽기 전용이라
...
다섯 AI 가 검토한 리뷰 4편이 실려 있다. 리뷰의 지적을 반영해 `final-report.md` 를 **한 단계 더 정확하게**
다듬어라.
...
> 5 AI 리뷰(claude·codex·grok·kimi·deepseek) → claude 종합 → final-report.md 갱신
...
- **리뷰가 final-report.md 를 직접 쓰게 하는 것** — 5 AI 는 읽기 전용이다. 리뷰는 의견만 stdout 으로
