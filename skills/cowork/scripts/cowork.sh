#!/usr/bin/env bash
# cowork.sh — claude·codex·grok·kimi·agy + OpenCode(GLM)를 읽기 전용 분석가로 병렬 실행한다.
#
#   사용: bash .claude/skills/cowork/scripts/cowork.sh <slug> <분석 요청 프롬프트>
#   결과: .cowork/<slug>/{claude,codex,grok,kimi,glm,agy}-cowork.md
#
...
#   - 여섯 AI 는 동시에 돌린다(가장 느린 하나의 시간만 걸린다). OpenCode 는 `glm` 하나만 독립
#     `opencode run` 프로세스로 실행한다(2026-08-17 사용자 지시).
...
# OpenCode 분석 이름은 glm 하나다. agy 는 Antigravity CLI(Gemini 계열)로 OpenCode 경로가 아니다.
#
# 🛑 2026-08-17 사용자 지시: **OpenCode 로는 GLM-5.3 하나만 쓴다.** 그전까지 쓰던
#    deepseek(v4-pro)·minimax(m3)·qwen(3.6-plus) 세 모델은 여기서 제거했다. 되돌리려면 이 배열과
#    opencode_model_for()·run_glm()·문서의 산출물 트리를 함께 고쳐야 한다(한 곳만 고치면 어긋난다).
COWORK_AI_NAMES=(claude codex grok kimi glm agy)
COWORK_OPENCODE_NAMES=(glm)
COWORK_ONLY="${COWORK_ONLY:-claude,codex,grok,kimi,glm,agy}"
...
#   - OpenCode: **GLM-5.3 하나만** 독립 `opencode run` 프로세스로 부른다(2026-08-17 사용자 지시).
#               구독 카탈로그에 glm-5.1·5.2·5.3 이 있고 그중 5.3 이 최신이다(실측 조회 확인).
#               🛑 프로바이더 접두어가 둘이니 주의 — `opencode-go/`(정액 구독)와
#               `opencode/`(Zen 종량제)는 다른 경로다. 반드시 opencode-go 를 쓴다. 접두어를 틀리면
#               "모델이 구독에 없다"로 그 AI 만 조용히 죽는다(2026-08-10 gemini 사고).
#               COWORK_OPENCODE_MODEL 은 예전 단일 설정의 하위 호환 별칭으로 계속 받는다.
#               ⚠️ glm 은 예전 감사에서 413s 로 다른 OpenCode 모델의 3배 이상 느렸다(당시 glm-5.2).
#               OpenCode 가 하나뿐인 지금은 그것이 곧 OpenCode 몫의 대기 시간이다.
COWORK_OPENCODE_GLM_MODEL="${COWORK_OPENCODE_GLM_MODEL:-${COWORK_OPENCODE_MODEL:-opencode-go/glm-5.3}}"
...
#
# 🛑 1번은 **분석 대상 폴더 안**이다. sandbox-exec 는 *쓰기만* 막고 읽기는 허용하므로 여섯 AI 가
...
check_opencode_key() {
  ...
  # 작업공간 안에 있으면 여섯 AI 가 전부 읽을 수 있다 — 샌드박스는 쓰기만 막는다.
  ...
  case "$kf_abs" in
    "$REPO_ROOT_PHYS"/*)
      ...
      printf '    여섯 AI 가 이 파일을 읽어 분석 출력에 흘릴 수 있다(샌드박스는 쓰기만 막는다).\n' >&2
...
# OpenCode 모델 ID 가 **실제로 구독에 존재하는지** 실행 전에 확인한다.
#
# 🛑 2026-08-10 실측 사고: `opencode-go/gemini-3.6-flash` 를 기본값으로 박아 뒀는데 OpenCode Go
#    구독에 gemini 계열이 아예 없어(16개 모델 중 0개) **매 실행마다 그 하나만 3초 만에 조용히 죽었다.**
...
환경변수:
  ...
  COWORK_ONLY           실행할 AI 목록(기본 claude,codex,grok,kimi,glm,agy)
  ...
  COWORK_OPENCODE_GLM_MODEL
                        OpenCode 로 부를 유일한 모델(기본 opencode-go/glm-5.3).
                        예전 COWORK_OPENCODE_MODEL 도 하위 호환 별칭으로 계속 지원한다.
                        독립된 opencode run 프로세스 하나로 실행된다.
  COWORK_OPENCODE_KEYFILE
                        opencode API 키 파일. 지정하지 않으면 아래 두 경로를 순서대로 찾아
                        먼저 실재하는 쪽을 쓴다:
                          1) <작업공간>/.cowork/opencode-go-api.key  (로컬 — 여섯 AI 가 읽을 수
                             있으므로 이 배치를 쓰면 실행할 때마다 경고한다)
                          2) ~/.config/cowork/opencode-go-api.key    (권장 — 작업공간 밖)
                        없거나 비면 glm 분석만 실패로 기록되고 나머지는 계속 간다.
...
예:
  ...
  COWORK_ONLY=glm,agy COWORK_TIMEOUT=600 bash .claude/skills/cowork/scripts/cowork.sh login-timeout "..."
...
#
# 🛑 시그널은 **프로세스 그룹 전체**에 보낸다(`kill -$pid` = PGID 가 $pid 인 그룹). 직접 자식에게만
#    보내던 예전 코드는 손자를 놓쳐 고아 프로세스를 쌓았다 — 2026-08-17 실측 사고:
#    opencode 좀비 **84개가 최대 4일간 살아남아 RSS 36.3GB** 를 점유했고(모델별 qwen 36·deepseek 33·
#    minimax 13), opencode 가 띄운 bash-language-server 까지 6시간째 고아로 떠 있었다. 그 메모리
#    압박이 새 분석의 응답을 지연시켜 "opencode 가 느리다" 로 오진되기 쉬웠다.
#    구조: 우리가 부르는 것은 `sandbox-exec` → CLI(node) → 그 CLI 의 자식들이다. 중간만 죽이면
#    나머지가 부모 없이 살아남는다.
#
# 방법:
#   1) 자식이 `setpgrp(0,0)` 으로 **자기 자신을 리더로 하는 새 그룹**을 만든다 → 손자도 같은 그룹.
#   2) 타임아웃 시 `-$pid` 로 그룹 전체에 TERM → KILL.
#   3) **정상 종료 뒤에도** 같은 정리를 한 번 더 한다 — CLI 가 0 으로 끝나도 LSP 서버 같은 자식을
#      남기는 일이 실재하기 때문이다(위 bash-language-server).
#
# 안전: `-$pid` 는 "PGID == 자식 PID" 인 그룹만 가리킨다. 부모(perl)의 PGID 는 셸의 것이라 절대
#       그 그룹에 속하지 않으므로 자기 자신을 죽일 위험이 없다. setpgrp 가 실패하면 그런 그룹이
#       없어 kill 이 조용히 실패할 뿐, 엉뚱한 프로세스를 죽이지 않는다.
run_timeout() {
  local secs="$1"; shift
  perl -e '
    my $s = shift;
    ...
    local $SIG{ALRM} = sub { $reap->(2); exit 124 };
    alarm $s;
    waitpid($pid, 0);
    my $rc = $? >> 8;
    alarm 0;
    $reap->(0.3);   # 정상 종료여도 남은 손자를 쓸어 담는다
    exit($rc);
  ' "$secs" "$@"
}
...
> 🛑 이 파일은 \`cowork\` 이 claude·codex·grok·kimi·glm·agy **여섯 AI 에게 분석을
...
do_review() {
  ...
  # 리뷰 프롬프트: 리뷰 페르소나 + final-report.md 전문 + 6개 원본 분석 전문
  ...
  {
    printf '## 리뷰 라운드 — %s\n\n' "$stamp"
    printf '> 6 AI 리뷰(claude·codex·grok·kimi·glm·agy) → claude 종합 → final-report.md **%s**\n\n' "$applied"
...
# --- OpenCode (독립 CLI 프로세스) --------------------------------------------
# 지금은 glm 하나뿐이지만 이 함수는 **이름·모델을 인자로 받는 일반 러너**로 남겨 둔다 —
# 모델을 다시 늘릴 때 run_<이름> 래퍼만 추가하면 되도록(2026-08-17 이전에는 세 모델이 이 함수를 공유했다).
# 메인 병렬 루프가 run_glm 을 백그라운드로 띄우고, 이 함수가 정확히 한 번의 `opencode run` 을 실행한다.
...
run_opencode_analysis() {
  ...
  if ! check_opencode_key; then
    ...
    printf '  1) %s/.cowork/opencode-go-api.key   (작업공간 로컬 — 여섯 AI 가 읽을 수 있으니 주의)\n' "$REPO_ROOT" >> "$log"
    printf '  2) ~/.config/cowork/opencode-go-api.key   (권장 — 작업공간 밖이라 여섯 AI 가 못 읽는다)\n\n' >> "$log"
...
run_glm() { run_opencode_analysis glm "$COWORK_OPENCODE_GLM_MODEL"; }
