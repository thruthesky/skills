...
## 💡 팀장(A) 제안
<아래 "팀장(A) 제안 섹션 작성 규칙" 을 따라 관찰된 맥락에 맞는 제안 0~N개>
```
...
`status` · `who` · `lock` 서브커맨드 결과 말미에는 **팀장(A) 제안** 섹션을 포함한다. 이 섹션은 팀 매니저(팀원 A) 관점에서 관찰된 맥락을 사용자에게 요약·조언하는 **권고(advisory)** 이며, **지시나 자동 실행이 아니다**.

### 포함해야 할 제안 카테고리 (관찰된 경우에만)

1. **인프라 도입 여부 결정** — `tools/team/`, `tools/lock/`, `.dev/team/logs/` 중 하나라도 부재하면, 단일 세션 환경인지 다중 세션 환경인지 확인한 뒤 정식 구축 필요성을 사용자에게 제시
   - 예: "팀 협업 인프라(`tools/team/`, `tools/lock/`, `.dev/team/logs/`)를 정식 구축할지 여부는 사용자 결정 사항입니다. 현재는 단일 세션 개발 환경으로 운영 중이며, 다중 Claude 세션 동시 작업이 없다면 굳이 필요하지 않을 수 있습니다."

2. **대기 해소 필요** — 특정 팀원이 `BLOCKER` 이벤트를 기록했거나, `TASK_START` 후 장시간 멈춰 있거나, Handoff 선결 조건을 기다리고 있으면 어떤 결정·정보가 필요한지 구체적으로 제시
   - 예: "팀원 B가 착수 가능하려면 옵션 A/B/C 선택과 '회원 상태별' 의도 확인이 필요합니다."

3. **미커밋 변경 처리** — `git status --short` 에 현재 분석·작업 맥락과 무관해 보이는 변경이 남아 있으면 커밋/discard 판단이 필요함을 알림. **본 명령은 읽기 전용이므로 직접 조치하지 않음** 을 반드시 함께 명시
   - 예: "현재 세션의 미커밋 변경: `pages/recent-posts.php`가 수정되어 있는데 분석 작업과 무관합니다. 커밋 또는 discard 여부 판단이 필요합니다 (본 명령은 읽기 전용이므로 조치하지 않음)."

4. **Lock 장기 보유 경고** — 특정 subject 를 15 분 이상 동일 팀원이 보유 중이면 stale 여부를 사용자에게 확인 요청 (force-release 여부는 사용자 결정)

5. **로그 누락 감지** — `LOCK_ACQUIRE` 만 있고 `LOCK_RELEASE` 가 없거나, `TASK_START` 만 있고 60 분 넘게 `TASK_PROGRESS`/`TASK_DONE` 이 없는 팀원이 있으면 점검 요청

6. **그 외 매니저 A 가 팁으로 공유할 가치가 있는 관찰** — 예: "팀원 C 가 최근 3 개 태스크에서 Lock 없이 CLI 간접 검증만으로 완료. 이 패턴을 다른 팀원에게도 권장 가능"

### 작성 원칙

- 제안 수는 **0 ~ N 개**. 관찰된 맥락이 없으면 해당 섹션을 비우거나 "제안 없음" 으로 표시
- 한 제안 = **한두 문장**. 사용자가 즉시 판단 가능한 수준의 구체성 유지
- 제안 끝에 **결정 주체가 사용자임** 을 명시하는 어구를 포함 (`… 여부는 사용자 결정 사항입니다`, `… 판단이 필요합니다`)
- 자동 실행·지시 금지 (본 명령의 읽기 전용 원칙 유지)
- 제안은 **매니저 A(팀원 A) 관점** 으로 작성. "팀장(A) 제안" 이라는 섹션 제목을 사용하며, 팀원 본인의 지시가 아님을 분명히 한다

### 출력 예시

```markdown
## 💡 팀장(A) 제안
- **인프라 도입 여부 결정**: 팀 협업 인프라(`tools/team/`, `tools/lock/`, `.dev/team/logs/`)를 정식 구축할지 여부는 사용자 결정 사항입니다. 현재는 단일 세션 개발 환경으로 운영 중이며, 다중 Claude 세션 동시 작업이 없다면 굳이 필요하지 않을 수 있습니다.
- **대기 해소 필요**: 팀원 B가 착수 가능하려면 옵션 A/B/C 선택과 "회원 상태별" 의도 확인이 필요합니다.
- **현재 세션의 미커밋 변경**: `pages/recent-posts.php`가 수정되어 있는데 분석 작업과 무관합니다. 커밋 또는 discard 여부 판단이 필요합니다 (본 명령은 읽기 전용이므로 조치하지 않음).
```
